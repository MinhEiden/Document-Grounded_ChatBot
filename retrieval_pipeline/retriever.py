...
def retrieve_context(query: str, k: int = 5) -> list[Document]:
    ...
    # Khởi tạo dữ liệu cho BM25 từ ChromaDB
    db_data = db.get()
    all_documents = []
    ...
    # Đưa kết quả vào một danh sách (ưu tiên vector rồi đến bm25)
    for doc in vector_docs + bm25_docs:
        if doc.page_content not in seen_contents:
            seen_contents.add(doc.page_content)
            unique_docs.append(doc)
            
    # Nếu không có tài liệu nào, trả về rỗng
    if not unique_docs:
        return []
    ...
    for doc in reranked_docs:
        # Cohere Rerank thường lưu relevance_score dạng float vào thuộc tính metadata
